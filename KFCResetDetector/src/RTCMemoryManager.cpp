...
bool RTCMemoryManager::write(RTCMemoryId id, const void *dataPtr, uint8_t dataLength)
{
    ...
    MUTEX_LOCK_BLOCK(_lock) {
        ...
        uint8_t *outPtr;
        auto memUniquePtr = std::unique_ptr<uint8_t[]>(_readMemory(header, dataLength));
        ...
        result = RTCMemoryManagerNS::system_rtc_mem_write(header.start_address(), reinterpret_cast<uint32_t *>(memPtr), header.length);
    }
