...
SyslogFile::SyslogFile(SemaphoreMutex &lock, const char *hostname, const String &filename, size_t maxSize, uint16_t maxRotate) :
...
{
    PrintString buffer;
    _addTimestamp(buffer, millis(), PSTR(SYSLOG_FILE_TIMESTAMP_FORMAT));
    _addParameter(buffer, _parameter.getHostname());
    buffer += F(SYSLOG_APPNAME ": ");
    return buffer;
}

void SyslogFile::transmit(const SyslogItem &message)
{
    MUTEX_LOCK_BLOCK(_lock) {
        #if DEBUG_SYSLOG
            if (!message.startsWith(F("::transmit '"))) {
                __LDBG_printf("::transmit msg=%s", message.c_str());
            }
        #endif
