...
namespace ConfigurationHelper {
    ...
    {
        __LDBG_assertf(size != 0, "allocating size=%u", size);
    ...
    inline void allocate(size_t size, ConfigurationParameter &parameter)
    {
        __LDBG_assertf(size != 0, "allocating size=%u", size);
        #if 1
            __LDBG_assertf(parameter.hasData() == false, "has_data=%u is_writable=%u _readable=%p", parameter.hasData(), parameter.isWriteable(), parameter._getParam()._readable);
        ...
        __LDBG_assertf(size >= param.size(), "size=%u too small param.size=%u", size, param.size());
        size_t realSize;
